...
public class Main_BJ_17144_미세먼지안녕_박서영 {
	...
	private static void clockwise() {
		...
		for(int d=0; d<4; d++) {
			while(true) {
				...
				map_clone[nx][ny] = map[x][y];
				x = nx;
				y = ny;
			}
		}

		map_clone[aircleaner1][0] = -1;
		map_clone[aircleaner1][1] = 0;
		for(int i=0; i<R; i++)
			map[i] = map_clone[i].clone();
	...
	private static void counterclockwise() {
		...
		map_clone[aircleaner2][0] = -1;
		map_clone[aircleaner2][1] = 0;
		for(int i=0; i<R; i++)
			map[i] = map_clone[i].clone();
	}
